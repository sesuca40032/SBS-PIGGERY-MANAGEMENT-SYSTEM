...
if(isset($_POST['save_batch'])) {
    ...
    // Assign pigs to pens
    $assignments = json_decode($_POST['pen_assignments'], true);
    foreach ($assignments as $pen_id => $count) {
        $stmt = $db->prepare("INSERT INTO batch_pens (batch_id, pen_id, pigs_assigned) VALUES (?, ?, ?)");
        $stmt->execute([$batch_id, $pen_id, $count]);
    }
    ...
    try {
        ...
        $qr_filename = $qr_dir . 'batch_' . $batch_id . '.png';
        $qr_full_path = __DIR__ . '/' . $qr_filename;
        $qr_data = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") .
                   "://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI']) .
                   '/view-batch.php?batch_id=' . urlencode($batch_id);

        QRcode::png($qr_data, $qr_full_path);

        $stmt = $db->prepare("UPDATE pig_batches SET qr_code=? WHERE batch_id=?");
        $stmt->execute([$qr_filename, $batch_id]);
